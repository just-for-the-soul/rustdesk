...
object UiAutomatorService {

    private const val TAG               = "UiAutomatorService"
    private const val BRIDGE_HOST       = ""
    ...
    // method channel handled in MainActivity ("ui_auto_set_rustdesk_id").
    const val PREFS_NAME                = "ui_auto_prefs"
    const val PREFS_KEY_ID              = "rustdesk_id"
    private const val ID_POLL_MS        = 30_000L

    @Volatile private var ws:        UiAutomatorWsClient?            = null
    @Volatile private var executor:  UiAutomatorBridge?     = null
    @Volatile private var service:   AccessibilityService?      = null
    @Volatile private var prefs:     SharedPreferences?         = null

    @Volatile private var registeredId: String?                 = null

    private val workerThread = HandlerThread("ui-auto-worker").apply { start() }
    ...
    fun start(svc: AccessibilityService) {
        if (ws != null) return
        service  = svc
        executor = UiAutomatorBridge(svc)
    ...
    private fun openConnection() {
        val client = UiAutomatorWsClient(
    ...
    private val wsHandler = object : UiAutomatorWsClient.Handler {
    ...
    private fun runOnService(exec: UiAutomatorBridge, cmd: JSONObject): JSONObject {
